$env.command_not_found_handler = { |cmd_name|
    ...
    if ($r.0 | str starts-with "The") {
        let pkgs = $r | skip 1 | append '' | str join ", after running pkg install main-" | split row "or, after running pkg install main-" | str trim | parse --regex 'pkg install (?P<package>\S*), after running pkg install (?P<repo>\S*)-' | table
        ...
        let pkgs = $r | skip 1 | append '' | str join ' from the main-;' | split row ';' | str trim | parse --regex 'Command (?P<command>\S*) in package (?P<package>\S*) from the (?P<repo>\S*)-' | table
